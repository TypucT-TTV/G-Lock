...
def get_public_ip() -> str:
    # Called at import time by menu.menu's class body (before any window can
    # appear, GUI or console) — must not hang or raise if the network is
    # unreachable, only degrade gracefully (an empty string just never
    # matches a real packet's source IP in the known_allowed set it feeds).
    try:
