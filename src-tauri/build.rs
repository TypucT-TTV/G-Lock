            ...
            .and_then(|p| p.parent())
        {
            ...
            if dll_src.exists() {
                std::fs::copy(&dll_src, &dll_dest)
                    .expect("failed to copy WinDivert.dll to target directory");
            }
            if sys_src.exists() {
                std::fs::copy(&sys_src, &sys_dest)
                    .expect("failed to copy WinDivert64.sys to target directory");
            }
            if db_src.exists() {
                std::fs::copy(&db_src, &db_dest)
                    .expect("failed to copy db.json to target directory");
            }
        }
    }
}
